...
| CMake 3.30+ | 构建（`build.py` 会在 `.venv` 中安装） |
...
source scripts/init-build-env.sh    # Clang 20、MKL、CUDA 路径
```

`build.py` 首次运行时会创建 `.venv` 并安装 `py_visualizer/requirements.txt`（含 `cmake>=3.30`、numpy、matplotlib 等）。
